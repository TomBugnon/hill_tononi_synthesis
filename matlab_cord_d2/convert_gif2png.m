...
for i=1:1:output_frame_num
    ...
    try 
        % Load image
        src_filename = strcat(dirname,'/',prefix,'.gif');
        [I,map] = imread(src_filename,'gif', frame_idx);
        % Display image
        %imshow(I,map)
    ...
    % If target is white area, activate the following section
    % --- Because NEST code will pick up high high intensity pixels(<-this is
    % --- black area in "small_img", see the comment for one section above)
    if strcmp(target, 'white')
        dst_img = abs(dst_img - 255);       
    end
    ...
    %--- Save png image
    dst_filename = strcat(dst_folder_name, prefix, sprintf('%04d',i), '.png');
    ...
    scrbl_xy_filename = strcat(scrbl_xy_folder_name, prefix, sprintf('%04d',i), '.png');
    imwrite(scrbl_xy_img,scrbl_xy_filename);
                           
    %--- Scramble temporal information
    scramble_timestamp = rand_t_idx(i);
    scrbl_t_filename = strcat(scrbl_t_folder_name, prefix, sprintf('%04d',scramble_timestamp), '.png');
