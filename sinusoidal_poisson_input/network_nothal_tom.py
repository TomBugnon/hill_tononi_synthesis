...
def get_Models(params):
    ...
    # test Tom ### CRITICAL
    nest.SetDefaults('ht_synapse', {'tau_P': 50., 'delta_P': 0.2})
    # nest.SetDefaults('ht_neuron', params = { 'instant_unblock_NMDA' : True , 'S_act_NMDA' : 1./2.5, 'V_act_NMDA' : -58.})
...
def get_Connections(params):
    ...
    Vp_horizontal_intralaminar_base = {
        ...
        "kernel": {"gaussian": {"p_center": 0.05/p_ratio, "sigma": 7.5 * dpcP}},
    ...
    Vp_vertical_interlaminar_base = {
        ...
        "kernel": {"gaussian": {"p_center": 1.0/p_ratio, "sigma": 7.5 * dpcP}},
    ...
    Vp_intracortical_inhibitory_base = {
        ...
        "mask": {"circular": {"radius": 7.0 * dpcP}},
        "kernel": {"gaussian": {"p_center": 0.25/p_ratio, "sigma": 7.5 * dpcP}},
        # "weights": 1.0, # original
        "weights": 1.0 * weight_gain, #
    ...
    Vs_vertical_interlaminar_base = {
        ...
        "kernel": {"gaussian": {"p_center": 1.0/p_ratio, "sigma": 7.5 * dpcS}},
        #"weights": 2.0, # original
        #"weights": 2.0 * weight_gain,
    ...
    Vs_intracortical_inhibitory_base = {
        ...
        "synapse_model": "GABA_A_syn",
        "mask": {"circular": {"radius": 7.0 * dpcS}},
        "kernel": {"gaussian": {"p_center": 0.25/p_ratio, "sigma": 7.5 * dpcS}},
        #"weights": 1.0,  # original
        "weights": 1.0 * weight_gain,
        #"weights": 1.5 * weight_gain, # keiko
        "delays": {"uniform": {"min": 1.75, "max": 2.25}},
    ...
    for conn in [{"sources": {"model": "L23_inh"}, "targets": {"model": "L23_exc"},
                  ...
                  "synapse_model": "GABA_B_syn", "mask": {"circular": {"radius": 2.0 * dpcS}}, "kernel": 0.3/p_ratio},  #keiko
    ...
    Vs_Vp_backward_interareal_base = {
        ...
        "mask": {"circular": {"radius": 12.0 * dpcP}},
        "kernel": {"gaussian": {"p_center": 0.1/p_ratio, "sigma": 7.5 * dpcP}},
        #"weights": 1.0, # original
        "weights": 1.0 * weight_gain,
    ...
    # Horizontally tuned
    Vp_retina_base.update({"mask": {"rectangular": {"lower_left" : [-4.0*dpcP, -1.0*dpcP],
                                                             "upper_right": [ 4.0*dpcP,  1.0*dpcP]}}})
    for conn in [{"targets": {"model": "L4_exc" }, "kernel": 0.5},
                 {"targets": {"model": "L56_exc"}, "kernel": 0.3}]:
        Vp_retina_base.update(conn)
        allconns.append(['Retina_layer','Vp_horizontal', Vp_retina_base.copy()])

    # Vertically tuned
    Vp_retina_base.update({"mask": {"rectangular": {"lower_left" : [-1.0*dpcP, -4.0*dpcP],
                                                             "upper_right": [ 1.0*dpcP,  4.0*dpcP]}}})
    for conn in [{"targets": {"model": "L4_exc" }, "kernel": 0.5},
                 {"targets": {"model": "L56_exc"}, "kernel": 0.3}]:
        Vp_retina_base.update(conn)
        allconns.append(['Retina_layer','Vp_vertical', Vp_retina_base.copy()])
    ...
    for conn in [{"targets": {"model": "L4_inh" }},
                 {"targets": {"model": "L56_inh"}}]:
        Vp_Thalamocortical_diff_base.update(conn)
        allconns.append(['Retina_layer','Vp_horizontal', Vp_retina_diff_base.copy()])
        allconns.append(['Retina_layer','Vp_vertical', Vp_retina_diff_base.copy()])



    #
    #
    # nest.CopyModel('static_synapse', 'retinal_projection',
    #                params={'receptor_type': receptors['AMPA']})
    # retVp = {
    #     "connection_type": "divergent",
    ...
    #
    #
    # for conn in [{"targets": {"model": "L4_exc"}},
    #              {"targets": {"model": "Tp_inh"}}]:
    #     retVp.update(conn)
    #     # allconns.append(['Retina_layer','Tp_layer', retThal.copy()])
